...
import {
  ...
  COLOR_NIGHT, COLOR_FOG, COLOR_SODIUM, COLOR_FILL, COLOR_AMBER,
} from './constants.js';
...
function initEngine() {
  ...
  // Modest lift so ACES keeps night warmth without crushing midtones.
  renderer.toneMappingExposure = 1.28;
  renderer.outputColorSpace = THREE.SRGBColorSpace;

  scene = new THREE.Scene();
  // Rooftop night: sodium / graffiti gold. Not cool blue, not FIFA daylight.
  ...
  scene.add(new THREE.AmbientLight(COLOR_SODIUM, 0.38));
  scene.add(new THREE.HemisphereLight(COLOR_AMBER, 0x3d2210, 0.58));

  const sun = new THREE.DirectionalLight(COLOR_SODIUM, 1.18);
  ...
  const fill = new THREE.DirectionalLight(COLOR_FILL, 0.48);
